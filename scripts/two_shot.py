...
class Script(scripts.Script):
    ...
    def title(self):
        return "Latent Couple extension for API"
    ...
    def process_sketch(self, img_arr, input_binary_matrixes):
    ...
    def update_mask_filters(self, alpha_blend_val, general_prompt_str, *cur_weights_and_prompts):
    ...
    def ui(self, is_img2img):
            ...
            with gr.Accordion("Latent Couple", open=False):
                ...
                with gr.Tabs(elem_id="script_twoshot_tabs") as twoshot_tabs:
                    ...
                    with gr.TabItem("Mask", elem_id="tab_twoshot_mask") as twoshot_tab_mask:
                        ...
                        button_run.click(self.process_sketch, inputs=[canvas_image, binary_matrixes],
                                         outputs=[post_sketch, binary_matrixes, alpha_mask_row, alpha_color, *color_row, *colors],
                                         queue=False)

                        button_update.click(fn=self.update_mask_filters, inputs=[alpha_blend, general_prompt, *cur_weight_sliders, *prompts], outputs=[final_prompt, alpha_mask_row, alpha_color, *colors])
        ...
        process_script_params.append(enabled)               #ダミー
    ...
    def process(self, p: StableDiffusionProcessing, *args, **kwargs):
        ...
        enabled, raw_divisions, raw_positions, raw_weights, raw_end_at_step, alpha_blend, base64_img, *cur_weight_sliders = args
        ...
        if type(base64_img) is str:
            image_data = base64.b64decode(base64_img)
            image = Image.open(BytesIO(image_data)).convert("RGB")
            image = np.array(image)
            
            binary_matrixes = []
            self.process_sketch(image, binary_matrixes)
            weight_and_dummy_prompt = cur_weight_sliders + [""] * MAX_COLORS
            self.update_mask_filters(alpha_blend, "", *weight_and_dummy_prompt)
